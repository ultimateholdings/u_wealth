...
<?php
$sql	= "SELECT *, sum(amount) as total_amm FROM `earning` GROUP by userid ORDER BY `total_amm` DESC LIMIT 50";
$top_earn	= $this->db->query($sql)->result_array();
?>
<div class="content">
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        <div class="card">
           <div class="card-header card-header-icon ">
                                  <div class="card-icon"><i class="fa fa-user"></i></div>
            <h4 class="card-title ">Leaders</h4>
            <p class="card-category">Here is the list of members who have earned the most</p>
          </div>
          <div class="card-body">
            <div class="table-responsive">
              <table class="table">
                ...
                <tbody>
                <?php $sn = 1;//count($top_earn);
                    foreach ($top_earn as $e) { ?>
                        <tr>
                            <td><?php echo $sn++; ?></td>
                            <td>
                                <a href="<?php echo site_url('users/user_detail/' . $e['id']) ?>"
...
<script type="text/javascript">
      $(document).ready(function(){
        var cDate=new Date();
        document.getElementById("datetime").innerHTML = cDate.toLocaleString();
      });
    </script>