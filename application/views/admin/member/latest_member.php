...
<div class="table-responsive">
    <table class="table table-striped table-bordered" style="font-size:13px;margin-top: 10px;" id="DTable" data-page-length='100' data-name="member_list" data-export='Yes'>
        <thead>
        <tr>
            <th>SN</th>
            <th>User ID</th>
        ...
        foreach ($members as $e) { ?>
            <tr>
                ...
                <!--    <?php if (config_item('enable_topup') == "Yes" && $e['topup'] <= 0) { ?>
                    ...
                    <a href="<?php echo site_url('users/user_detail/' . $e['id']); ?>" class="btn btn-warning btn-sm">View</a>
                    <a href="<?php echo site_url('users/edit_user/' . $e['id']); ?>" class="btn btn-info btn-sm">Edit</a>
...
<a href="<?php echo site_url('admin') ?>" class="btn btn-xs btn-danger" style="margin-top: 10px;">&larr; Go Back</a>

<script type="text/javascript">
    $(document).ready(function () {
        document.getElementsByClassName('active')[0].classList.remove('active');
        document.getElementById("members").classList.add('active');
        document.querySelector("#members > ul > li:nth-child(4) > a > span").setAttribute('style', 'color: darkorange !important;');
    });
</script>