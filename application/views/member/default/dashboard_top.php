...
<div class="alert alert-light">
  <div class="row">
    <div class="col-sm-2">
      <button class="btn mr-3 btn-sm btn-secondary ng-star-inserted" style="color: #fff;background-color: #0000FF;border-color: #ff4081;height:50px;">Latest News</button>
    </div>
    <div class="col-sm-10">
     <marquee direction="left" onmouseover="this.stop();" onmouseout="this.start();">
...
<?php if($this->db_model->count_all('member', array('position<='=>0,'sponsor'=>$member_data['member']->id, 'status'=>'Active'))>0){ ?>
    <?php if($member_data['member']->position>0||$member_data['member']->id==config_item('top_id')){?>
        <div class="alert alert-success" >
        You have Received Request to place users in Tree !!! Please <a href="<?php echo site_url('tree/holdinglist') ?>" style='color: blue;'> Click Here </a> to View !!!
        </div>
    <?php } else { ?>
        <div class="alert alert-danger" >
        Please contact to your sponsor to place in the Tree 
        </div>
    <?php } ?>
<?php } ?>

<?php if($member_data['member']->position<=0&&$member_data['member']->id!=config_item('top_id')&&$member_data['member']->status=='Active'){?>
        <div class="alert alert-danger" >
        Please contact to your sponsor to place in the Tree 
        </div>
<?php }?>
             ...
             <div class="profile-content" >
                ...
                <?php if (config_item('enable_based_pv') == "Yes"){ ?>
                    <?php if($this->session->user_id != config_item('top_id')){ ?>
                        ...
                        <h4 style="color: blue; font-size:16px;"><span style="color: black;">Plan : </span><?php echo !empty($package_details->plan_name) ? $package_details->plan_name : $member_data['pd']->plan_name ?> </h4>
                ...
                <?php } else if($member_data['member']->status == 'Inactive') { ?>
                    ...
                    if (config_item('product_activate') == "Yes")
                    { ?>
                      <a href="<?php echo site_url('store') ?>" style="color: blue;">&nbsp;<u>Purchase a product to Activate Your Account</u></a>
