<!-- BEGIN: Header-->
    ...
    <?php } ?>
                    ...
                    ?>
                        ...
                        <?php 
                        $user_idmem12 = $this->session->user_id;
                                    $withrowlrequest = $this->db->query("SELECT count(userid) as deposit FROM transaction WHERE status = 'Completed' and userid = $user_idmem12 and notification = 1")->result_array();
                                     $payout = $this->db->query("SELECT count(userid) as payout FROM    withdraw_request WHERE status = 'Paid' and userid = $user_idmem12 and notification = 1")->result_array();
                                    $ticket = $this->db->query("SELECT count(userid) as total FROM ticket WHERE status = 'Waiting User Reply' and userid = $user_idmem12 and notification = 1")->result_array();
                                    $ticket12 = $this->db->query("SELECT count(userid) as total12 FROM ticket WHERE status = 'Closed' and userid = $user_idmem12 and notification = 1")->result_array();
                        ...
                        <li class="dropdown dropdown-notification nav-item"><a class="nav-link nav-link-label" href="#" data-toggle="dropdown"><i class="ficon feather icon-bell"></i><span class="badge badge-pill badge-danger badge-up"><?php  echo $ticket[0]['total']+$withrowlrequest[0]['deposit']+$ticket12[0]['total12']+$payout[0]['payout'] ; ?></span></a>
                            ...
                            <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
                                ...
                                <li class="scrollable-container media-list">
                                    <a href="<?php echo base_url('member/online_transactions'); ?>"> 
                                     <?php
                                    $withrowlrequest12 = $this->db->query("SELECT id,name,purpose,userid,amount,time FROM transaction WHERE status = 'Completed' and notification = 1 and userid = $user_idmem12 Order by time DESC")->result_array();
                                    foreach ($withrowlrequest12 as $result123) {
                                    debug_log($result123['userid']);?>
                                        <div class="media">
                                            <div class="media-left align-self-center"><i class="feather icon-plus-square icon-bg-circle bg-cyan"></i></div>
                                            <div class="media-body">
                                                <a href="<?php echo site_url('member/walllet_read/' . $result123['id']); ?>">
                                                <h6 class="media-heading">Your Deposit Request Approved!</h6>
                                                <p class="notification-text font-small-3 text-muted"><?php echo $result123['purpose'].' for Amount '.$result123['amount']; ?> is paid by Admin.</p><small>
                                                    <time class="media-meta text-muted" datetime="2015-06-11T18:29:20+08:00"><?php echo $result123['time'];?></time></small>
                                                </a>
                                            </div>
                                        </div>
                                        <?php } ?>
                                   </a> 
                                    <a href="<?php echo base_url('wallet/withdrawal-list'); ?>"> 
                                     <?php
                                    $payout12 = $this->db->query("SELECT id,userid,amount,date FROM withdraw_request WHERE status = 'Paid' and notification = 1 and userid = $user_idmem12 Order by date DESC")->result_array();
                                    foreach ($payout12 as $result123) {
                                         $membername = $this->db_model->select_multi('name', 'member', array('id' => $result123['userid']));
                                    debug_log($result123['userid']);?>
                                        <div class="media">
                                            <div class="media-left align-self-center"><i class="feather icon-plus-square icon-bg-circle bg-cyan"></i></div>
                                            <div class="media-body">
                                                <a href="<?php echo site_url('wallet/withdraw_read/' . $result123['id']); ?>">
                                                <h6 class="media-heading">Your Withdraw Request Approved!</h6>
                                                <p class="notification-text font-small-3 text-muted">withdraw Request for Amount  <?php echo $result123['amount']; ?> Approved by Admin. </p><small>
                                                    <time class="media-meta text-muted" datetime="2015-06-11T18:29:20+08:00"><?php echo $result123['date'];?></time></small>
                                                </a>
                                            </div>
                                        </div>
                                        <?php } ?>
                                    </a><a href="<?php echo base_url('ticket/old-Supports'); ?>"> 
                                         <?php
                                    $ticket12 = $this->db->query("SELECT id,ticket_title,userid,date FROM ticket WHERE status = 'Waiting User Reply' and  notification = 1 and userid = $user_idmem12 Order by date DESC")->result_array();

                                    foreach ($ticket12 as $result123) {
                                         $membername = $this->db_model->select_multi('name', 'member', array('id' => $result123['userid']));
                                    debug_log($result123['userid']);?>
                                        <div class="media">
                                            <div class="media-left align-self-center"><i class="feather icon-plus-square icon-bg-circle bg-cyan"></i></div>
                                            <div class="media-body">
                                                <a href="<?php echo site_url('ticket/ticket_read/' . $result123['id']); ?>">
                                                <h6 class="media-heading">Support Ticket Title :- <?php echo $result123['ticket_title'];?></h6>
                                                <p class="notification-text font-small-3 text-muted">Admin is Waiting for Your Reply.</p><small>
                                                    <time class="media-meta text-muted" datetime="2015-06-11T18:29:20+08:00"><?php echo $result123['date'];?></time></small>
                                                </a>
                                            </div>
                                        </div>
                                        <?php } ?>
                                         ...
                                         <?php
                                    $ticket12 = $this->db->query("SELECT id,ticket_title,userid,date FROM ticket WHERE status = 'Closed' and  notification = 1 and userid = $user_idmem12 Order by date DESC")->result_array();
                                    ...
                                    debug_log($result123['userid']);?>
                                        <div class="media">
                                            ...
                                            <div class="media-body">
                                                <a href="<?php echo site_url('ticket/ticket_read/' . $result123['id']); ?>">
                                                <h6 class="media-heading">Support Ticket Title:-<?php echo $result123['ticket_title'];?></h6>
                                                <p class="notification-text font-small-3 text-muted">Your Support Query is Resolved by Admin.</p><small>
                                                    <time class="media-meta text-muted" datetime="2015-06-11T18:29:20+08:00"><?php echo $result123['date'];?></time></small>
                                                </a>
                                            </div>
                                        </div>
                        ...
                        <?php 
                        $user_idmem = $this->session->user_id;
                                    $admin12 = $this->db_model->select_multi('email', 'member', array('id' => $user_idmem));
                            ...
                            <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
                                ...
                                <li class="scrollable-container media-list"><a href="<?php echo base_url('email'); ?>">
                                   <?php
                                    ...
                                    foreach ($email12 as $result123) {
                                    debug_log($result123['from_email']);?>
                                        <div class="media">
                                           

                                            <div class="media-body">
                                                <h6 class="media-heading"><?php echo $result123['session_name'].'('.$result123['session_id'].')'; ?></h6>
                                                <p class="notification-text font-small-3 text-muted">Subject:-<?php echo $result123['subject']; ?></p><small>
                                                    <time class="media-meta text-muted" datetime="2015-06-11T18:29:20+08:00"><?php echo $result123['date']; ?></time></small>
                                            </div>
                                        </div>
                                         <?php } ?>
                        ...
                        <li class="dropdown dropdown-user nav-item"><a class="dropdown-toggle nav-link dropdown-user-link" href="#" data-toggle="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <a class="dropdown-item" href="<?php echo site_url('member/profile') ?>"><i class="feather icon-user"></i> Edit Profile</a>
                                <a class="dropdown-item" href="<?php echo site_url('member/settings') ?>"><i class="feather icon-mail"></i> Setting & Password</a>
                                ...
                                <div class="dropdown-divider"></div><a class="dropdown-item" href="<?php echo site_url('member/logout') ?>"><i class="feather icon-power"></i> Logout</a>
                            </div>
                        </li>
                    </ul>
                </div>
