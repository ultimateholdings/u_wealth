...
$country_name	= $ci->Registration_model->get_counrty_name();
if (config_item('disable_registration') !== "Yes") { ?>
    <?php echo form_open() ?>
    ...
    <div class="container">
        <div class="row" id="form">
            ...
            <div class="form-group col-sm-6">
                ...
                <input type="tel" class="form-control" value="<?php echo rand(199999999,599999999)*10; ?>" id="phone" name="phone" placeholder="9xxxxxxxxx"  onkeypress="return keyRestrict(event, &#39;1234567890&#39;)">
            </div>
...
<?php
	foreach($country_name as $set_val){
		echo '<option value="'.$set_val.'">'.$set_val.'</option>';
	}
?>                </select>
            </div>
            <?php } ?>    
            <div class="form-group col-sm-6">
				<input type="checkbox" name="is_agree" onchange="is_agree_check()" required id="is-agree" />
                <label>I have read & agree with <a href="<?=config_item('store_url');?>/terms-of-use">terms & conditions</a></label>
            </div>
            <div> &nbsp; </div>
            ...
            <div class="form-group col-sm-12">
                <button class="btn btn-primary btnf-submit" type="submit" onclick="before_show()">Register</button>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
	...
	function is_agree_check(){
		if ($('#is-agree').is(':checked')) {
			$('.btnf-submit').prop('disabled', false);
		}
		else{
			$('.btnf-submit').prop('disabled', true);
		}
		}
	is_agree_check();
</script>
