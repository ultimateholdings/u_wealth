...
class Plan_model extends CI_Model
{
    ...
    public function unlimited_cycle_upline($md,$pd,$cs)
    {
        ...
        $upline='';
        if(config_item('crowdfund_type')=='Manual_Peer_to_Peer')
        $ud=$this->db_model->select('sponsor','member',array('id'=>$userid ,'signup_package'=>$pd->id));
        else
        ...
        while (strlen($position)>2) 
        {
          ...
          if(config_item('crowdfund_type')=='Manual_Peer_to_Peer')
          $position = $this->db_model->select('sponsor','member',array('id'=>$position ,'signup_package'=>$pd->id));
          else
        ...
        $count= 1;
        #$width = $pd->max_width;
        #$config = ($pd->max_width==3) ? config_item('unlimited_cycle_level') : 3 ;
        ...
        {   
          ...
          if($count==$config)
          {
            if(config_item('crowdfund_type')=='Manual_Peer_to_Peer')
            {
              $umd = $this->db_model->select_multi('*', 'member', array('id' => $value));
              if ($umd->gift_level>$md->gift_level) {
                debug_log('executed');
                $upline_id =  $value;
                break;
              }
              $config = $config + config_item('unlimited_cycle_level'); 
            }
            else
            {
              debug_log('executed');
  ...
  public function create_tree_unilevel($id, $above_id = '', $position = 'A',$plan='')
  {
    $my_tree = 'my_unilevel_tree';
        ...
        else
        {
            ...
            if ((config_item('leg') == '2')) {
                ...
                if(config_item('enable_pv')=="Yes") {
                    ...
                    $total_node = $total_node . 'Total Binary Pairs: ' . $data->mypv/3 . '<br/> ';
                    $total_node = $total_node . 'Total Left Carry Pairs: ' . $data->total_a_pv/3 . '<br/> ';
                    $total_node = $total_node . 'Total Right Carry Pairs: ' . $data->total_b_pv/3 . '<br/> ';
                    /*$total_node = $total_node . 'Total Binary Points: ' . $data->binary_points . '<br/> ';
                    $total_node = $total_node . 'Total Pairs: ' . $data->binary_points/3 . '<br/> ';
                    $total_node = $total_node . 'Total Left Carry Points: ' . $data->left_carry_points . '<br/> ';
                    

                    $total_node = $total_node . 'Total Right Carry Points: ' . $data->right_carry_points . '<br/> ';*/
